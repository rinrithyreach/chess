...
/**
 * What the bot's seat was called by a build that let you choose its strength.
 *
 * A game saved then still carries one of these. storage.js gives that seat the
 * plain name back as the record loads, so nothing reading a save afterwards
 * has to know the bot was ever called anything else.
 */
export const RETIRED_BOT_NAMES = ['Easy', 'Medium', 'Hard']
  .map((label) => `${BOT_NAME} (${label})`);
